"""Bluetooth transport for the embedded Radiacode library."""
...
class ConnectionClosed(Exception):
    """Raised when the Bluetooth connection is closed."""
    pass
...
if platform.system() == 'Darwin' or _have_bluetooth is False:
    ...
    class Bluetooth:
        """Dummy Bluetooth class for unsupported platforms."""
        
        def __init__(self, mac: str, poll_interval: float = 0.01):
            """Initialize dummy Bluetooth transport.
            
            Args:
                mac: Bluetooth MAC address (ignored on unsupported platforms)
                poll_interval: Poll interval (ignored on unsupported platforms)
            """
            raise NotImplementedError("Bluetooth is not supported on this platform")

        def close(self):
            """No resources to release on unsupported platforms."""
            pass
    ...
    class Bluetooth(DefaultDelegate):
        """Bluetooth transport for Radiacode devices."""
